...
\begin{frame}{Formalisierung}
  ...
  \item<3-> gdw. $\exists x(Gx \land \forall y(Gy \to x=y))$.
  \end{itemize}
\end{frame}
...
\begin{frame}{Semantik: Interpretation}
  ...
  $\langle D,v\rangle$.
  \begin{itemize}[<+->]
  \item $D$ ist der (nicht-leere) Gegenstandsbereich, über den
    quantifiziert wird.
...
\begin{frame}{Semantik: Wahrheit in einer Interpretation}
  ...
  \item $v(\exists xA) = 1$ gdw. \textbf{mindestens ein} Objekt des
    Gegenstandsbereiches $A$ erfüllt, sonst 0.
    \begin{itemize}
    \item Eine Formel $A$ gilt in $I$ gdw. $v(A) = 1$.
...
\begin{frame}{Semantik: Interpretation}
  ...
  \begin{itemize}[<+->]
  \item $D$ wie in der klassischen Prädikatenlogik ein nicht-leerer
    Gegenstandsbereich ist,
  \item $W$ wie in der modalen Aussagenlogik eine nicht-leere Menge
    möglicher Welten ist,
...
\begin{frame}{Barcan Formula: Beispiel}
  ...
  \item An anderen Welten soll es ruhig Dinge geben können, die ein
    Perpetuum Mobile sind.
  \end{itemize}
\end{frame}

\begin{frame}{Barcan Formula}
  \begin{itemize}[<+->]
  \item Aber dann beißt uns die Barcan Formula:
    \begin{center}
      BF: $\forall x \Box A \to \Box \forall x A$.
    \end{center}
...
\end{frame}
\begin{frame}{Barcan Formula: Responses}
  Um Constant Domain zu verteidigen könnte man auch sagen:
  \begin{enumerate}[<+->]
  \item[2.] Unser intuitives Verständnis der Quantoren ist
    ...
    \begin{itemize}[<+->]
    \item Allerdings braucht man sehr gute unabhängige Gründe, um so
      ein kontraintuitives Resultat für eine philosophische Theorie
      plausibel zu machen. Das heißt aber nicht, dass es die nicht
      geben kann! (Vergleiche: Es gibt kein Bier mehr!)
    \end{itemize}
  \end{enumerate}
\end{frame}
...
\begin{frame}{Eine mögliche Lösung: Variable Domain}
  ...
  \item Semantik: Interpretation $I$ wie vorher 4-Tupel
    $\langle D, W, R, v \rangle$, allerdings wird $v$ verändert, so
    dass:
    \begin{itemize}[<+->]
    \item Wenn $w \in W$, dann $v(w) \subseteq D$.
      \begin{itemize}[<+->]
      \item D.h. der Gegenstandsbereich kann von Welt zu Welt
        variieren.
      \end{itemize}
    \item Wenn $a$ Konstante, dann $v(a) \in D$.
      \begin{itemize}[<+->]
      \item Das bleibt gleich: Eine Konstante bezeichnet immer
        dasselbe Objekt.
      \end{itemize}
    \item Wenn $P$ n-stelliges Prädikatsymbol, dann $v_w(P) \in D^n$.
      \begin{itemize}[<+->]
      \item Und nicht $v_w(P) \in v(w)^n$. Dadurch erlauben wir, dass
        Dinge auch Prädikate an einer Welt erfüllen können, ohne dass
        sie dort existieren!
      \end{itemize}
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Variable Domain}

Wahrheit in einer Interpretation genau wie vorher, nur:
  \begin{itemize}[<+->]
  \item $v_w(\forall x A) = 1$ gdw. jedes $\partial \in \mathbf{v(w)}$
    Formel $A$ erfüllt.
  \item $v_w(\exists x A) = 1$ gdw. mindestens ein
    $\partial \in \mathbf{v(w)}$ Formel $A$ erfüllt.
...
\begin{frame}{Ein Argument gegen Existenz eines Objekts an mehreren Welten}
  ...
  \pause
  \begin{itemize}[<+->]
  \item Wie kann überhaupt ein und dasselbe Objekt in verschiedenen
    Welten existieren? Es hat ja dann in einer Welt Eigenschaften, die
    es in einer anderen Welt nicht hat, also simultan eine Eigenschaft
    $P$ und nicht die Eigenschaft $P$! Das kann für \emph{alle}
    Eigenschaften gelten, die es hat! Kann es dann \emph{dasselbe}
    Objekt sein?
...
\begin{frame}{Lewisian Counterparts}
  ...
  \begin{itemize}[<+->]
  \item Extreme Version: David Lewis' Counterparts-Theorie. Ein Ding
    existiert in nur einer Welt, und steht mit maximal ähnlichen
    Dingen an anderen Welten in einer Counterpart-Relation. Modalität
    wird dann zusätzlich über die Counterpart-Relation erzeugt. Die
    muss jedoch nicht symmetrisch oder transitiv sein!
  \item Resultierende Logik um einiges schwächer, z.B. gilt selbst in
    S5 dann nicht mehr $\Box p \to \Box\Box p$ u.a.
  \end{itemize}
...
\begin{frame}{Alles halb so wild..}
  ...
  \item Dann hat er in dieser Welt die Eigenschaft, potentiell ein
    Demokrat zu sein.
  \item Kein Widerspruch!
  \item Stattdessen vielleicht epistemisches Problem? Wie können wir
    wissen, welches Objekt in einer anderen Welt dasselbe ist wie in
    dieser, wenn die Eigenschaften alle anders sind?
  \end{itemize}
\end{frame}
...
\begin{frame}{Weitere Probleme}
  ...
  Vergleiche:
  \begin{itemize}[<+->]
  \item Notwendigerweise ist das Verhältnis von Umfang und Durchmesser
    eines Kreises irrational.
